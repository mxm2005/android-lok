...
public class T9Adapter extends BaseAdapter implements Filterable
{
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		else
		{
			...
			holder.number.setText(Html.fromHtml(list.get(position).getPhoneNum()
				...
				for(int i = 0; i < filterNum.length(); i++)
				{
					...
					if(TextUtils.isDigitsOnly(String.valueOf(c)))
					{
						...
						if(zms != null)
						{
							for(char c1 : zms)
							{
								formattedNumber = formattedNumber.replaceAll(String.valueOf(c1)
										.toUpperCase(), "%%" + String.valueOf(c1).toUpperCase()
										+ "@@");
							}
